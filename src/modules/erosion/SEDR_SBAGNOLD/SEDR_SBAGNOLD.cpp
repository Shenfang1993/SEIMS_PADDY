...
void  SEDR_SBAGNOLD::initialOutputs()
{
    ...
    {       
        ...
        for (int i = 1; i <= m_nreach; i++){
			...
			//m_sedStorage[i] = m_sedChi0 * m_preChStorage[i] * 1000.f; /// ton/m3 * m3/m * m * 1000 = kg
			m_sedStorage[i] = 0.01 * m_chStorage[i] * 1000.f; /// ton/m3 * m3/m * m * 1000 = kg
...
void SEDR_SBAGNOLD::SedChannelRouting(int i)
{
    ...
    float peakFlowRate = m_qchOut[i] * 1.5f;
