...
{
	if (m_surfaceRunoff[cell] < 0.0001f)
		return 0.f;
	else
...
//#pragma omp parallel for
    ...
    for (int i = 0; i < m_nCells; i++)
    {
        ...
        else
        {
			...
			// the response of sediment to flow seems abnormal, so do the limit
			/*if(m_sedimentYield[i] > 1.f){
			m_sedimentYield[i] = m_sedimentYield[i] / 1000.f;
			}*/
		}
