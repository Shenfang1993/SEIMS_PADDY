...
#define FieldFileName "mgt_fields"

using namespace MainBMP;

BMPArealStructFactory::BMPArealStructFactory(int scenarioId, int bmpId, int subScenario, int bmpType, int bmpPriority,
...
/*
	...
	if(templateRaster != NULL) delete templateRaster;
	
}

void BMPArealStructFactory::loadBMP(mongoc_client_t *conn, string &bmpDBName)
{
...
void BMPArealStructFactory::BMPParametersPreUpdate(map<string, clsRasterData<float>*> rsMap, int nSubbasin,  mongoc_gridfs_t *spatialData)
{
	...
	vector<int> locationArr = utils::SplitStringForInt(this->m_location, ',');

	ostringstream oss;
	oss << nSubbasin << "_" << GetUpper(FieldFileName);
	templateRaster = new clsRasterData<float>(spatialData,oss.str().c_str());
	readFieldRasterfile(nSubbasin, spatialData, templateRaster);
...
void BMPArealStructFactory::readFieldRasterfile(int nSubbasin, mongoc_gridfs_t* spatialData, clsRasterData<float>* templateRaster)
{
	// read spatial data from mongDB
	int n = 0;
	clsRasterData<float>*raster = NULL;
	ostringstream oss;
	try
	{
		oss << nSubbasin <<"_"<<GetUpper(FieldFileName);
		raster = new clsRasterData<float>(spatialData, oss.str().c_str(), templateRaster);
	}
	...
	//m_rsMap[FieldFileName] = raster;  //map<string, clsRasterData*>
