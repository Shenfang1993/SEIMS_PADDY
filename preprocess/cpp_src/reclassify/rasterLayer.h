...
readFile(const char *inputfile)
{
    ...
    poBandsrc->RasterIO(GF_Read, 0, _pMetaData->_MBR.minIRow(), _pMetaData->_localdims.nCols(),
                        _pMetaData->_localdims.nRows(), _pCellSpace->_matrix, _pMetaData->_localdims.nCols(),
                        _pMetaData->_localdims.nRows(), _pMetaData->dataType, 0, 0);
...
createFile(const char *outputfile)
{
    ...
    if (_pMetaData->myrank == 0)
    {
        ...
        GDALDataset *poDataset = poDriver->Create(outputfile, _pMetaData->_glbDims.nCols(),
                                                  _pMetaData->_glbDims.nRows(), 1, _pMetaData->dataType, papszMetadata);
        if (poDataset == NULL)
        {
            //do something
    ...
    else
    {
        if (_pMetaData->myrank == 0)
        {
            ...
            poBanddest->RasterIO(GF_Write, 0, 0, _pMetaData->_glbDims.nCols(), _pMetaData->_localworkBR.maxIRow() + 1,
                                 _pCellSpace->_matrix, _pMetaData->_glbDims.nCols(),
                                 _pMetaData->_localworkBR.maxIRow() + 1, _pMetaData->dataType, 0, 0);
        }
        else if (_pMetaData->myrank == (_pMetaData->processor_number - 1))
        {
            ...
            poBanddest->RasterIO(GF_Write, 0, _pMetaData->_MBR.minIRow() - _pNbrhood->minIRow(),
                                 _pMetaData->_glbDims.nCols(), _pMetaData->_localworkBR.maxIRow() + 1,
                                 _pCellSpace->_matrix - _pNbrhood->minIRow() * _pMetaData->_glbDims.nCols(),
                                 _pMetaData->_glbDims.nCols(), _pMetaData->_localworkBR.maxIRow() + 1,
                                 _pMetaData->dataType, 0, 0);
        ...
        {
            poBanddest->RasterIO(GF_Write, 0, _pMetaData->_MBR.minIRow() - _pNbrhood->minIRow(),
                                 _pMetaData->_glbDims.nCols(),
                                 _pMetaData->_localworkBR.maxIRow() + _pNbrhood->minIRow() + 1,
                                 _pCellSpace->_matrix - _pNbrhood->minIRow() * _pMetaData->_glbDims.nCols(),
                                 _pMetaData->_glbDims.nCols(),
                                 _pMetaData->_localworkBR.maxIRow() + _pNbrhood->minIRow() + 1, _pMetaData->dataType, 0,
                                 0);
        }
    }
